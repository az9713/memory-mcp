...
class MigrationTests(unittest.TestCase):
    ...
    def test_migration_002_adds_scope_and_index_and_backfills(self) -> None:
        conn = _conn()
        conn.execute(
            """
            CREATE TABLE memories (
        ...
        conn.execute(
            ...
            " VALUES ('m1', 'x', 'warm', 0.5, 1, 1)"
        )
        conn.commit()

        memory_server._migration_002_add_scope(conn)

        scope = conn.execute("SELECT scope FROM memories WHERE id = 'm1'").fetchone()["scope"]
        self.assertEqual(scope, "global")

        idx = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_memories_scope_tier_last_accessed'"
        ).fetchone()
        self.assertIsNotNone(idx)

    def test_migration_002_is_safe_to_rerun(self) -> None:
        conn = _conn()
        conn.execute(
            """
            CREATE TABLE memories (
        ...
        scope_cols = [r["name"] for r in rows if r["name"] == "scope"]
        self.assertEqual(scope_cols, ["scope"])
